...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        ...
        if message.chat.id in temp.BANNED_CHATS:
            ...
            return
        buttons = [[
            InlineKeyboardButton('ℹ️ 𝙷𝙾𝚆 𝚃𝙾 𝚄𝚂𝙴 𝙼𝙴', url=f"https://t.me/{temp.U_NAME}?start=help"),
...
async def gen_invite(bot, message):
    ...
    try:
        ...
        return await message.reply('Give Me A Valid Chat ID')
    try:
        link = await bot.create_chat_telegram.ChatInviteLink(chat)
    except ChatAdminRequired:
        return await message.reply("Invite Link Generation Failed, Iam Not Having Sufficient Rights")
    except Exception as e:
        return await message.reply(f'Error {e}')
    await message.reply(f'Here is your Invite Link {link.telegram.ChatInviteLink}')
