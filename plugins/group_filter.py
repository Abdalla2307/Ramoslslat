...
async def auto_filter(client, msg, spoll=False):
    ...
    if SHORT_URL and SHORT_API:          
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_caption}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}"))] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_caption}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}")),
        ...
        if settings["button"]:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {file.file_caption}", callback_data=f'{pre}#{req}#{file.file_id}')] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_caption}", callback_data=f'{pre}#{req}#{file.file_id}'),
    ...
    if imdb:
        ...
        cap = f"✅ <b>مسلسلك جاهز يــا {message.from_user.mention}\n⚡الحلقات الجديدة هتنزل يوميا لو مش لاقيها استني شوية وابحث تاني</b>"
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("<b>𝟷. تــأكد من كتابة المسلسل بالطريقة الصحيحة مثل\n<i>  مسلسل الكبير - مسلسل الاجهر - مسلسل الطيارة</i>\n\n𝟸. لو مش لاقي المسلسل الادمن هيضيفه في اسرع وقت</b>", reply_to_message_id=msg.id)
