...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏩ الحلقات القديمة", callback_data=f"next_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [InlineKeyboardButton(f"🗓 {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("الحلقات الجديدة ⏪", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
...
async def pm_next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⏩ الحلقات القديمة", callback_data=f"pmnext_{req}_{key}_{off_set}"),
